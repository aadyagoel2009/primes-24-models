...
# Function to randomly zero out class importances for a selected class
def zero_out_class_importance(class_importances, y_train_labels):
    # Randomly select a class
    selected_class = random.choice(list(set(y_train_labels)))
    print(f"Randomly chosen class: {selected_class}")
    
    # Set the importance of all nodes (words) for the selected class to 0
    for word in class_importances:
        class_importances[word][selected_class] = 0
...
def main():
    ...
    # Randomly select a class and zero out the importance for that class
    selected_class = zero_out_class_importance(class_importances, y_train_labels)
    ...
    print(f"Accuracy on entire test data: {accuracy_all * 100:.2f}%")

    # Filter out test data for the selected class
    filtered_test_indices = [i for i, label in enumerate(y_test_labels) if label != selected_class]
    filtered_test_vectors = doc_vectors_test[filtered_test_indices]
    filtered_test_labels = y_test_labels[filtered_test_indices]

    # Predict on the filtered test set
    y_pred_filtered = svm.predict(filtered_test_vectors)

    # Evaluate the classifier on the filtered test set (excluding chosen class)
    accuracy_filtered = accuracy_score(filtered_test_labels, y_pred_filtered)
    print(f"Accuracy excluding documents from class {selected_class}: {accuracy_filtered * 100:.2f}%")
